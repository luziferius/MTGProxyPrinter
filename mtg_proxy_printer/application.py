...
import mtg_proxy_printer.ui.progress_bar
...
class Application(QApplication):
    ...
    def enqueue_startup_tasks(self, args: Namespace):
        ...
        is_first_start = mtg_proxy_printer.settings.is_first_start()
        if is_first_start:
            logger.debug("First application start detected, will create and show the first start wizard.")
            self.first_launch_wizard = self._create_first_launch_wizard(
        ...
        elif not self.card_db.has_data() and not is_first_start:
        ...
        if args.file is not None:
            if not self.card_db.has_data():
                logger.warning("Can’t load save file from command line, because the card database is empty.")
    ...
    def _create_first_launch_wizard(
        ...
        wizard.card_data_download_requested.connect(
            card_info_downloader.import_from_api)
