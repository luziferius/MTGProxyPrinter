...
HTTPFileFixture = Callable[[], MeteredSeekableHTTPFile]

@pytest.fixture
def http_file() -> HTTPFileFixture:
    def create():
        with patch("mtg_proxy_printer.http_file.MeteredSeekableHTTPFile._urlopen",
            ...
            return mtg_proxy_printer.http_file.MeteredSeekableHTTPFile("")
    return create


def set_file_size(http_file: HTTPFileFixture, size: int):
...
def test_read_content_length_with_file(http_file: HTTPFileFixture, expected_size: int):
...
def test_read_content_length_without_file(http_file: HTTPFileFixture):
    http_file.file = None
    expected_size = -1
    assert_that(http_file._read_content_length(http_file.file), is_(expected_size))
...
def test___enter___emits_io_begin_signal(
        http_file: HTTPFileFixture, qtbot: QtBot, ui_hint: str, expected_size: int):
    set_file_size(http_file, expected_size)
...
def test___exit___emits_signals_on_regular_closure(
        http_file: HTTPFileFixture, qtbot: QtBot, expected_size: int):
    set_file_size(http_file, expected_size)
...
def test___exit___emits_signals_on_exception_during_closure(
        http_file: HTTPFileFixture, qtbot: QtBot, expected_size: int, error_on_exit: typing.Type[Exception]):
    set_file_size(http_file, expected_size)
...
def test_tell(http_file: HTTPFileFixture, pos: int):
    http_file._pos = pos
...
def test_seek_raises_error_if_connection_is_not_seekable(http_file: HTTPFileFixture, offset: int, whence: int):
...
def test_seek_moves_to_expected_position(
        http_file: HTTPFileFixture,  offset: int, whence: int, start_pos: int, expected: int):
...
def test_readinto1(http_file: HTTPFileFixture, size: int):
...
@pytest.mark.parametrize("size", [1, 10])
def test_read1(http_file: HTTPFileFixture, size: int):
    buffer = bytearray(size)
...
@pytest.mark.parametrize("size", [1, 10])
def test_readline(http_file: HTTPFileFixture, size: int):
    buffer = bytearray(size)
...
@pytest.mark.parametrize("size", [1, 10])
def test_readlines(http_file: HTTPFileFixture, size: int, hint: int):
    line_length = 7
...
def test__store_and_report_read_progress(
        http_file: HTTPFileFixture, qtbot: QtBot, pos: int, total_read_bytes: int, progress: int):
    http_file._pos = pos
...
def test_close(http_file: HTTPFileFixture):
...
def test_close_twice(http_file: HTTPFileFixture):
    assert_that(http_file.closed, is_(False))
    http_file.close()
    http_file.close()
    assert_that(http_file.closed, is_(True))
...
def test_delegates(http_file: HTTPFileFixture, method):
...
def test_content_encoding_without_file(http_file: HTTPFileFixture):
    http_file.file = None
    assert_that(http_file.content_encoding(), is_(none()))


def test_content_encoding_with_file(http_file: HTTPFileFixture):
...
def test_seekable(http_file: HTTPFileFixture, content_length: int, ranges_header_value: str, expected: bool):
...
def test_read_raises_value_error_if_closed(
        http_file: HTTPFileFixture, size: int, exception_class):
    http_file.close()
...
def test_read_re_raises_error_on_exceeded_retries(
        http_file: HTTPFileFixture, size: int, exception_class, retry: int):
...
@pytest.mark.parametrize("size", [1, 10])
def test_read_returns_bytes_from_internal_file(http_file: HTTPFileFixture, size: int):
    buffer = bytearray(size)
    http_file.file.read.return_value = buffer
    assert_that(http_file.read(size), is_(equal_to(buffer)))


def test_read_returns_incomplete_internal_reads(http_file: HTTPFileFixture):
    available = bytes(10)
...
def test_read_aborts_after_exhausting_retries(
        http_file: HTTPFileFixture, retries: int, exception_class):
...
@patch.multiple("mtg_proxy_printer.http_file.urllib.request", autospec=True, Request=DEFAULT, urlopen=DEFAULT)
def test__urlopen_includes_range_header_when_seeking_to_non_zero_pos(
        urlopen: MagicMock = None, Request: MagicMock = None):
    file = MeteredSeekableHTTPFile("")
