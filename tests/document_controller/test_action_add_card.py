...
@pytest.fixture()
def check_card(card: Card, image_db: ImageDatabase) -> CheckCard:
...
@pytest.fixture()
def document_light_3(document_light: Document) -> Document:
...
def test_apply_emits_page_type_changed_when_page_type_changes(qtbot: QtBot, card: Card, document_light: Document):
    with qtbot.wait_signal(document_light.page_type_changed, check_params_cb=lambda x: x.row() == 0):
...
def test_apply_does_not_create_pages_with_mixed_card_sizes(
        document_light: Document, card: Card, oversized_card: Card, oversized_first: bool):
    if oversized_first:
...
def test_apply_does_not_overfill_oversized_pages(document_light: Document, oversized_card: Card):
...
def test_apply_only_emits_page_type_changed_signal_if_changed(
        qtbot: QtBot, document_light: Document, card: Card, oversized_card: Card, page_type: PageType):
...
def test_undo_works_when_apply_searched_over_multiple_full_pages_for_free_slot(document_light: Document, card: Card):
    """
