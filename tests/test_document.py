...
def test_save_as_saves_regular_card(document: Document, is_front: bool):
    card = document_custom_layout.card_db.get_card_with_scryfall_id("b3b87bfc-f97f-4734-94f6-e3e2f335fc4d", is_front)
    document.apply(ActionAddCard(card))
    with TemporaryDirectory() as temp_dir:
        save_file = pathlib.Path(temp_dir)/"test.mtgproxies"
        document_custom_layout.save_as(save_file)
        with sqlite_helpers.open_database(save_file, "document-v6", DocumentLoader.MIN_SUPPORTED_SQLITE_VERSION, False) as con:
            content = con.execute("SELECT page, slot, scryfall_id, is_front, type FROM Card").fetchall()
    assert_that(
        content, contains_exactly(
            contains_exactly(1, 1, "b3b87bfc-f97f-4734-94f6-e3e2f335fc4d", is_front, "r")
        )
    )
